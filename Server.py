    ...
    for respondent in longpoll.listen():
        try:
            if respondent.type == VkBotEventType.MESSAGE_NEW:
                ...
                # Шаблонные ответы
                edit_word.execute(
                    f"CREATE TABLE IF NOT EXISTS '{str(peerID)}' ( id INT, key TEXT PRIMARY KEY, val TEXT);")
                BDWORDS.commit()
                ...
                # ролевые команды
                edit_roles.execute(
                    f"CREATE TABLE IF NOT EXISTS '{str(peerID)}' ( id INT PRIMARY KEY, command TEXT ,emoji_1 TEXT, txt TEXT, emoji_2 TEXT);")
            ...
            send_to_specific_peer(f"{e}", respondent.object['peer_id'])
    ...
    for resend in longpoll_full.listen():
        try:
            ...
            if resend.type == VkBotEventType.MESSAGE_NEW:
                ...
                #######################################################################################
                if UserId > 0: parse_resend(node,resend.object,PeerId,UserId).sender()
        except Exception as e:
...
def vkNode():
        ...
        def TG_VK(message):
            ...
            if peer is not None: node = peer[0]#
            time.sleep(1)
            if node is not None: SendTGtoVK(message,node).upload_vk()
        try:
