...
from tools import logger, write_file, convert_img, TEXT_SPLIT, json_gen
...
class SendTG(object):
    ...
    def send_photo(self):
        if self.text > self.n:
            self.send_message()
        try: bot.send_media_group(chat_id=self.adress, media=self.att)
        except : bot.send_photo(chat_id=self.adress, photo=self.att[0],caption=self.text_sep)
        logger(f"{self.log}")
    ################################################################################################
    def send_audio(self):
        try:
            bot.send_media_group(chat_id=self.adress, media=self.att)
        except:  bot.send_message(chat_id=self.adress,text=f"{self.text_sep[0]}",disable_web_page_preview=False)
...
class SendTGtoVK(object):
    ...
    def is_it(self):
        if self.obj.text:
            ...
            else: write_file(self.name, bot.download_file((bot.get_file(self.id)).file_path))
            ...
            try:
                ...
                if sep == 'mp4':
                    ...
                    write_file(self.name, bot.download_file((bot.get_file(self.id)).file_path))
                    self.caption += f"\n{file_type}"
                    try:self.download = upload.document(doc=self.name, title=f"{name_file}.{file_type}",
