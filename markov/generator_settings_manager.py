...
class GenerateSettings:
    ...
    async def _check_rule(self, args):
        return (
            args
            and len(args) >= self.len_index
            and Patterns.pattern_bool(
                args[self.param_chance], [Patterns.chance_pattern]
            )
        )
    ...
    async def set_state(self, args):
        return await self._set_chance(
            args, "g_text_state", "Калибровка для генерации текста"
        )

    
    async def set_lstate(self, args):
        return await self._set_chance(
            args, "g_long_text_state", "Калибровка для генерации длинного текста"
        )
